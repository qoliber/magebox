...
func (m *DnsmasqManager) generateConfig() string {
	...
	// On macOS, listen on 127.0.0.1 (used by /etc/resolver/test)
	if m.platform.Type == platform.Linux {
		return `# MageBox DNS Configuration
# Routes *.test domains to localhost
...
# systemd-resolved will forward .test queries here
